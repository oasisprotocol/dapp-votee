...
import { POLL_CHOICES, PollChoice } from '../../constants/config.ts'
...
export const ResultsPage: FC = () => {
  ...
  const [data, dataValueSum, winningMascot, dataColorMap] = useMemo(() => {
    ...
    const _data = POLL_CHOICES.map((pollChoice, i) => ({ ...pollChoice, value: voteCount[i] }))
    const _dataOrdered = _data.sort(({ value: a }, { value: b }) => {
      if (a > b) {
        return -1
      } else if (a < b) {
        return 1
    ...
    const [winningDataPoint] = _dataOrdered
    const _winningMascot = POLL_CHOICES.find(({ name }) => name === winningDataPoint.name)!
    ...
    return [
      _dataOrdered as PollChoiceWithValue[],
